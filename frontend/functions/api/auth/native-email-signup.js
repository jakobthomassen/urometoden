...
import { createNativeSession } from '../../lib/session.js'
...
export async function onRequestPost({ request, env }) {
  ...
  if (existing) {
    ...
    // Google user adding password — link credential
    ...
    const result = await createNativeSession(existing, env)
  ...
  await logEvent(env, {
    ...
    tag:      'bruker',
    targetId: user.id,
    meta:     { email: user.email, source: 'mobile-email' },
  })

  const result = await createNativeSession(user, env)
  return Response.json(result)
}
